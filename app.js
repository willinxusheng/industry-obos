  ...
  function buildDetailOption(x) {
    ...
    if (x.forecast.median) {
      ...
      for (var k = 0; k < H; k++) {
        ...
        band.push(+(x.forecast.p75[k] - x.forecast.p25[k]).toFixed(1));
      }
    } else {
    ...
    document.getElementById('fcNote').innerHTML =
      ...
      '，实测覆盖率回到 50%）。主推演为<b>共识度自适应组合</b>：类比池共识高则多信类比、共识低则收敛至稳健"持平"基线；近邻选取还按波动/趋势体制匹配，偏向与当前市场体制相同的历史片段。推演的价值在方向与不确定性区间，<b>不在精确点位</b>（点位误差比"持平"基线好约 8.5%，且 Diebold-Mariano 块级检验 p&lt;0.05 显著）。';
